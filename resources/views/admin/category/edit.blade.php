            ...
            <div class="col-12 col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h4>{{ __('categories.update_category') }}</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.category.update', $category->id) }}" method="POST">
                                ...
                                <select class="form-control @error('language') is-invalid @enderror"
                                    ...
                                    @foreach($languages as $language)
                                        <option value="{{ $language->code }}"
                                                @selected(old('language', $category->language) == $language->code)>
                                            {{ $language->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <small class="form-text text-muted">{{ __('categories.slug_hint') }}</small>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('show_at_nav') is-invalid @enderror"
                                    ...
                                    <option value="">{{ __('common.select') }}</option>
                                    <option value="1" @selected(old('show_at_nav', $category->show_at_nav) == 1)>
                                        {{ __('common.yes') }}
                                    </option>
                                    <option value="0" @selected(old('show_at_nav', $category->show_at_nav) == 0)>
                                        {{ __('common.no') }}
                                    </option>
                                </select>
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save"></i> {{ __('categories.update') }}
                                </button>
