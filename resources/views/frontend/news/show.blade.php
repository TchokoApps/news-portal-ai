...
<section class="pt-4 pb-5">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <article>
                    <div class="mb-3 text-uppercase small text-muted">{{ $news->category?->name ?? 'General' }} | {{ strtoupper($language) }}</div>
                    <h1 class="mb-3">{{ $news->title }}</h1>
                    <div class="mb-4 text-muted">by {{ $news->author?->name ?? 'Admin' }} | {{ $news->created_at->format('M d, Y') }}</div>
                    <div class="mb-4">
